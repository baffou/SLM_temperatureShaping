...
function Qmap=uniformSquare(Nx,Ny,sqSize,smooth)

% Nx, Ny: Size of the image, in px.
%         Must be even numbers.
% sqSize: size of the uniform-temperature square, in px.
...
%% definition of the temperature field
%% over an area of interest
% in this example : uniform temperature = 1 over a square.

n=0;
for x=-Nx2+1/2:1:Nx2-1/2
    for y=-Ny2+1/2:1:Ny2-1/2
        if abs(x)<sqSize/2 && abs(y)<sqSize/2;
            n=n+1;
            ix=x+Nx2+1/2;
            iy=y+Ny2+1/2;
            Tmap(iy,ix)=1;
